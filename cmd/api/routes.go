...
func (app *application) routes() http.Handler {
	...
	mux.Get("/test-generate-token", func(w http.ResponseWriter, r *http.Request) {
		token, err := app.models.User.Token.GenerateToken(3, 60*time.Minute)
		if err != nil {
			app.errorLog.Println(err)
			return
		}
	...
	mux.Get("/test-save-token", func(w http.ResponseWriter, r *http.Request) {
		...
		user, err := app.models.User.GetById(3)
		if err != nil {
			app.errorLog.Println(err)
			return
		}
	...
	mux.Get("/test-validate-token", func(w http.ResponseWriter, r *http.Request) {
		...
		valid, err := app.models.Token.IsTokenValid(tokenToValidate)
		if err != nil {
			app.errorJSON(w, err)
			return
		}
